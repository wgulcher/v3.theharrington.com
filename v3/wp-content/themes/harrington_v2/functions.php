...
if (!function_exists('bootstrapwp_theme_setup')):
    function bootstrapwp_theme_setup() {
        ...
        require 'class-bootstrapwp_walker_nav_menu.php';
    }
endif;
...
// http://snippetrepo.com/snippets/restore-lost-capabilities-for-nextgen-gallery
...
// End function restore_lost_capabilities
...
class Bootstrap_Walker extends Walker_Nav_Menu     {     
        ...
        {    
			...
			if	(str_replace('-',' ',$parts[$i]) == str_replace('-',' ',strtolower($item->title)) ) {
				$item->classes[] = 'active';
				 break;
			}
		}	
            ...
            $attributes .= ! empty( $item->xfn )        ? ' rel="'    . esc_attr( $item->xfn        ) .'"' : '';
				if ($item->title == 'Book Now') {
			  $attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .
			  '?loadOBMApplication.action?siteId=FINEXHARR3&request_locale=en&chainAction=newAvailabilitySearch&arrival='.$day.'%2F'.$month.'%2F'.$year.'&numberOfPersons=1&numberOfNights=1'.'"' : '';
		   } else {			
            $attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .'"' : ''; }
            $item_output = $args->before; 
 
            if ($item->hasChildren && $depth == 0) { 
                $item_output .= '<a'. $attributes .'>';//  $item_output .= '<a'. $attributes .' class="dropdown-toggle" data-toggle="dropdown">'; 
            } else { 
            ...
            if ($item->hasChildren && $depth == 0) { 
              $item_output .= '</a>';   //$item_output .= '<b class="caret"></b></a>'; 
            } else { 
                $item_output .= '</a>'; 
            } 
...
// default
wp_enqueue_script('harrington-js', get_template_directory_uri() . '/assets/js/harrington.js', null, '0.1', false );
